...
describe("Testing Usuario Crud in MongoDB", () => {
  ...
  beforeAll(async () => {
    ...
    process.env = {
      ...env,
      MONGODB_URI: "mongodb+srv://javier15:Martinez15@mycluster.d3asc.mongodb.net/?retryWrites=true&w=majority",
      MONGODB_DB: "sw202202",
      MONGODB_SETUP: 1,
    };
  ...
  test('Insert One Record', async ()=>{
    const result = await CatDao.insertOne({ email:'Javier Menocal',nombre:'Gerson Martinez',avatar:'Gerson Martinez',password:'Gerson Martinez', estado:'ACT'});
    console.log(result);
  ...
  test('Update One Record', async ()=>{
    const updateResult = await CatDao.updateOne({codigo:id.toString(), email:'Gerson Martinez',nombre:'Gerson Martinez',avatar:'Gerson Martinez',password:'Gerson Martinez', estado:'INA'});
