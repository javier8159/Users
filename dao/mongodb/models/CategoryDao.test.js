...
describe("Testing Categoria Crud in MongoDB", () => {
  ...
  test('Insert One Record', async ()=>{
    const result = await CatDao.insertOne({ categoria:'Test  gm INS', estado:'ACT'});
    console.log(result);
  ...
  test('Update One Record', async ()=>{
    const updateResult = await CatDao.updateOne({codigo:id.toString(), categoria:'TEST gm INS UPD', estado:'INA'});
